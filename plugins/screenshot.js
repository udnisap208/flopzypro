...
if (Config.WORKTYPE == 'private') {
    ...
    XTroid.addCMD({pattern: 'ss ?(.*)', fromMe: true, desc: Lang.SS_DESC}, (async (message, match) => {
        ...
        var webimage = await axios.get(`https://shot.screenshotapi.net/screenshot?&url=${match[1]}&fresh=true&output=image&file_type=png&wait_for_event=load`, { responseType: 'arraybuffer' })
    ...
    XTroid.addCMD({pattern: 'ss ?(.*)', fromMe: true, desc: Lang.SS_DESC, dontAddCMDList: true}, (async (message, match) => {
        ...
        var webimage = await axios.get(`https://screenshotapi.net/api/v1/screenshot?url=${match[1]}&fresh=true&output=image&file_type=png&wait_for_event=load`, { responseType: 'arraybuffer' })
