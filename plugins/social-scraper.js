...
if (Config.WORKTYPE == "private") {
  XTroid.addCMD(
    { pattern: "insta ?(.*)", fromMe: true, desc: Lang.DESC },
    async (message, match) => {
      if (match[1] === "")
      ...
      var lasiimage = await axios.get(lasijson.thumbnail, {
        responseType: "arraybuffer",
      });

      await message.client.sendMessage(
        message.jid,
        Buffer.from(lasiimage.data),
        MessageType.image,
        { mimetype: Mimetype.png, caption: msg }
      );
    }
  );

  XTroid.addCMD(
    { pattern: "device ?(.*)", fromMe: true, desc: Lang.DESC },
    async (message, match) => {
      const { data } = await axios(
        `https://zenzapi.xyz/api/gsmarena?query=${match[1]}&apikey=a9a05974d30e`
      );
  ...
  XTroid.addCMD(
    { pattern: "insta ?(.*)", fromMe: false, desc: Lang.DESC },
    async (message, match) => {
      if (match[1] === "")
        return await message.client.sendMessage(
          message.jid,
